...
export default async function PurchaseOrderDetailPage(props: PageProps<"/purchases/[id]">) {
  const { id } = await props.params;
  const orderId = Number(id);

  const [order] = await callKw<OrderDetail[]>("purchase.order", "read", [
    [orderId],
  ...
  const [lines, invoices] = await Promise.all([
    callKw<OrderLine[]>("purchase.order.line", "read", [
      order.order_line,
  ...
  return (
    <div className="p-10">
      ...
      <div className="mt-3 flex items-start justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Order state and billing state are separate lifecycles — a
              confirmed ("Purchase") order still needs its own badge to show
              whether it's actually been billed, let alone paid. */}
          <StatusBadge value={order.invoice_status} />
          <PurchaseOrderActions orderId={order.id} state={order.state} hasInvoice={order.invoice_ids.length > 0} />
        </div>
      </div>
          ...
          <tbody>
            {lines.map((l) => (
              <tr key={l.id} className="border-b border-border last:border-0">
                ...
                <td className="tabular px-4 py-3 text-right font-mono">{l.product_qty}</td>
      ...
      {invoices.length > 0 && (
        <div className="mt-8">
          <h2 className="font-heading text-lg">Bills</h2>
                ...
                <div className="flex items-center gap-4">
                  ...
                  {inv.payment_state !== "paid" && inv.state === "posted" && (
                    <>
                      ...
                      <PaymentButton moveId={inv.id} redirectPath={`/purchases/${orderId}`} />
                    </>
                  )}
                </div>
              </div>
