...
export async function POST(req: Request) {
  ...
  const result = streamText({
    ...
    // live: a status lookup returned the right data but zero text). 5 steps
    // is enough for "resolve name -> propose -> narrate" chains without
    // letting a confused model loop indefinitely.
